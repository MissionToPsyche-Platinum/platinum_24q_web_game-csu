...
@onready var meltdown_bar: ProgressBar = $"Meltdown_UI/HBoxContainer/Meltdown Bar"
...
func _process(delta):
	if Game_State_Manager.in_meltdown:
		update_meltdown_timer()



func update_meltdown_timer():
	...
	meltdown_bar.value = progress_ratio * 100 # Set as percentage (0-100)
